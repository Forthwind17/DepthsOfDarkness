...
{
	public class SkyGuardianSummon : ModProjectile
	{
		...
		public override void AI()
		{
			...
			else
			{
				...
				Vector2 v2 = player.Center - center2 + new Vector2(0f, -45f);
			...
			float num45 = 10f;
			int num46 = ModContent.ProjectileType<SkyGuardianSummonProj>();
			if (!flag)
			{
				return;
			}
		...
		private bool CheckActive(Player player)
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<SkyGuardianSummonBuff>());

				return false;
			}

			if (player.HasBuff(ModContent.BuffType<SkyGuardianSummonBuff>()))
			{
