...
{
	public class ForgottenLightSummon : ModProjectile
	{
		...
		public override void AI()
		{
			...
			else
			{
				...
				v2.X -= 10 * Main.player[Projectile.owner].direction;
				v2.X -= num32 * 40 * Main.player[Projectile.owner].direction;
				v2.Y -= 10f;
			...
			if (Main.rand.NextBool(8))
				{
					int num40 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.Sandnado, 0f, 0f, 100, default(Color), 2f);
			...
			float num45 = 12f;
			int num46 = ModContent.ProjectileType<ForgottenLightSummonProj>();
			if (!flag)
			{
				return;
			}
		...
		private bool CheckActive(Player player)
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<ForgottenLightSummonBuff>());

				return false;
			}

			if (player.HasBuff(ModContent.BuffType<ForgottenLightSummonBuff>()))
			{
