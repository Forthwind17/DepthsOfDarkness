...
{
    public class CeobeKnifeProj1 : ModProjectile
    {
        ...
        {
            Dust dust = Dust.NewDustDirect(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.IceTorch, 0f, 0f, 255, default, 0.5f);
            dust.noGravity = true;
        ...
        {
            target.AddBuff(BuffID.Frostburn2, Main.rand.Next(60, 120), false);

            int p = Dust.NewDust(Main.rand.NextVector2FromRectangle(target.Hitbox), 0, 0, DustID.IceTorch, 0, 0, 0, default, 1.5f);
